...
def test_example_ts_generation(monkeypatch, tmp_path, capsys) -> None:
    ...
    assert exit_code == 0
    assert str(output) in captured.out
    assert "! r2scan-3c D4 def2-mTZVPP Opt" in text
    assert "{B 0 1 C}" in text
    assert "$new_job" in text
    assert "! r2scan-3c D4 def2-mTZVPP OptTS Freq" in text
...
def test_orca_extra_keywords_applied_to_int_and_ts(
    ...
    assert "! Opt Freq r2scan-3c D4 def2-mTZVPP TightSCF NormalSCF" in text_int
    ...
    assert "! r2scan-3c D4 def2-mTZVPP Opt TightSCF NormalSCF" in text_ts
    assert "! r2scan-3c D4 def2-mTZVPP OptTS Freq TightSCF NormalSCF" in text_ts
