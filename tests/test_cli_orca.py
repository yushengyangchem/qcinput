...
def test_example_sp_generation(monkeypatch, tmp_path, capsys) -> None:
    ...
    assert exit_code == 0
    assert str(output) in captured.out
    assert "! SP B3LYP def2-TZVP NoPop" in text
    assert "* xyz 0 1" in text
...
def test_orca_extra_keywords_applied_to_int_and_ts(
    ...
    assert "! Opt Freq B3LYP def2-TZVP TightSCF NormalSCF NoPop" in text_int
    ...
    assert "! B3LYP def2-TZVP Opt TightSCF NormalSCF NoPop" in text_ts
